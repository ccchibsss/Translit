...
# --- Ваш расширенный словарь брендов и моделей ---
car_brands_models = {
    # Вставьте сюда весь ваш расширенный словарь (так как он очень большой, я показываю как начало)
    "Acura": "Акура",
    "Integra": "Интегра",
    "MDX": "МДХ",
    # ... (весь ваш словарь)
    ...
    "Tesla": "Тесла",
    "Model 3": "Модель 3",
    # и так далее, вставьте сюда весь ваш словарь...
}

# Расширенный автоматический перевод en→ru
...
# Регулярное выражение для обработки токенов
_RE_TOK = re.compile(r'\w+|\s+|[^\w\s]+', flags=re.UNICODE)

# Ваша основная функция обработки текста
def process_text(
    text: str,
...
def process_file_for_processing(
    ...
    # Обрабатываем все строки в выбранном столбце
...
def run():
    ...
    edited_text = st.text_area("Редактировать словарь (латиница=кириллица)", value=dict_text, height=300)

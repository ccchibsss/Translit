...
_CYR_TO_LAT = {
    ...
    'П':'P','п':'p','Р':'R','р':'r','С':'S','с':'s','Т':'T','т':'т',
    'У':'U','у':'u','Ф':'F','ф':'ф','Х':'Kh','х':'kh','Ц':'Ts','ц':'ts',
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                if not df_preview.empty:
                    ...
                    col_name = st.selectbox(f"Выберите столбец для файла {uploaded_file.name}", options=col_options)
                    if col_name:
                        # Обработка файла
                        processed_df = process_file_for_processing(
                            file_bytes, uploaded_file.name, col_name,
                            st.session_state['car_brands_models'],
                            enable_lat_cyr
                        )
                        st.write(f"Обработанный файл для {uploaded_file.name}:")
